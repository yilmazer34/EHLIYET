...
class mAgst18VC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    var lessonName:String = "mAgst18"
    var point: String = " "
    
    var myCollectionView: UICollectionView!
    var questionsArray = [mAgst18]()
    var score: Int = 0
    var inCorrect: Int = 0
    var Correct: Int = 0
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "4 Ağustos 2018 Araç Te"
        ...
        myCollectionView.register(mAgst18VCell.self, forCellWithReuseIdentifier: "Cell")
        ...
        let que1 = mAgst18(imgName: "i18A1", questionText: "", options: ["A) Telaşlı ve tedirgin olması","B) İletişim becerilerinin zayıf olması","C) Önce çevrenin güvenliğini sağlaması","D) İnsan vücudu ile ilgili temel bilgilere sahip olması"], correctAns: 3, wrongAns: -1, isAnswered: false)
        let que2 = mAgst18(imgName: "i18A2", questionText: "", options: ["A) dolaşım","B) bilinç durumu","C) solunum","D) hava yolu açıklığı"], correctAns: 1, wrongAns: -1, isAnswered: false)
        let que3 = mAgst18(imgName: "i18A3", questionText: "", options: ["A) Yalnız I.","B) I ve II.","C) II ve III.","D) I, II ve III."], correctAns: 3, wrongAns: -1, isAnswered: false)
        let que4 = mAgst18(imgName: "i18A4", questionText: "", options: ["A) Şok pozisyonu","B) Yarı oturuş pozisyonu","C) Baş geri-çene yukarı pozisyonu","D) Yarı yüzükoyun-yan pozisyon"], correctAns: 2, wrongAns: -1, isAnswered: false)
        let que5 = mAgst18(imgName: "i18A5", questionText: "", options: ["A) 10","B) 20","C) 30","D) 40"], correctAns: 0, wrongAns: -1, isAnswered: false)
        let que6 = mAgst18(imgName: "i18A6", questionText: "", options: ["A) Kan şekeri düştüğünde","B) Tam tıkanma yaşadığında","C) Kanaması olduğunda","D) Kalbi durduğunda"], correctAns: 1, wrongAns: -1, isAnswered: false)
        let que7 = mAgst18(imgName: "i18A7", questionText: "", options: ["A) Kanamanın sızıntı şeklinde olması","B) Kanın koyu renkli ve küçük kabarcıklar şeklinde akması","C) Açık renkli kanın kalp atımları ile uyumlu olarak kesik kesik akması","D) Kanamanın fışkırma olmadan akması ve kısa sürede durdurulabilmesi"], correctAns: 2, wrongAns: -1, isAnswered: false)
        let que8 = mAgst18(imgName: "i18A8", questionText: "", options: ["A) Kanayan bölgeyi aşağI indirmek","B) Kanayan yer üzerine temiz bir bezle bastırmak","C) Uzuv kopması varsa boğucu sargı (turni-ke) uygulamak","Kanayan yere en yakın basınç noktasına baskı uygulamak"], correctAns: 0, wrongAns: -1, isAnswered: false)
        let que9 = mAgst18(imgName: "i18A9", questionText: "", options: ["A) Bilinç kontrolünün yapılması","B) Yaşam bulgularının sık sık izlenmesi","C) Dışarı çıkan organlarının içeri sokulmaya çalışılması","D) Bilinci yerinde ise sırtüstü pozisyonda bacakları bükülmüş olarak yatırılması"], correctAns: 2, wrongAns: -1, isAnswered: false)
        let que10 = mAgst18(imgName: "i18A10", questionText: "", options: ["A) Çıkığın yerine oturtulmaya çalışılması","B) Eklemin düzeltildikten sonra tespit edilmesi","C) Çıkık olan bölgeye kuvvetli masaj yapılması","D) Bölgenin nabzının, deri renginin ve ısısının kontrol edilmesi"], correctAns: 3, wrongAns: -1, isAnswered: false)
        let que11 = mAgst18(imgName: "i18A11", questionText: "", options: ["A) Koma","B) Bayılma","C) Sara krizi","D) Ateşli havale"], correctAns: 1, wrongAns: -1, isAnswered: false)
        let que12 = mAgst18(imgName: "i18A12", questionText: "", options: ["A) Rentek manevrası",
        ...
        let que13 = mAgst18(imgName: "i18A13", questionText: "", options: ["A) Karar verme ve kendini kontrol edebilme yeteneğinin azalması",
        ...
        questionsArray = [que1,que2,que3,que4,que5,que6,que7,que8,que9,que10,que11,que12,que13]
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell=collectionView.dequeueReusableCell(withReuseIdentifier: "Cell", for: indexPath) as! mAgst18VCell
    ...
    @objc func btnQuizFinishAction() {
        ...
        performSegue(withIdentifier: "mAgst18", sender: self)
    }
...
extension mAgst18VC: mAgst18VCellDelegate {
